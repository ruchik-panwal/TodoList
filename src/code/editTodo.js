  ...
  rmBtn.forEach((rm) => {
    rm.addEventListener("click", () => {
      const index = parseInt(
        rm.parentElement.parentElement.parentElement.id.slice(1),
      ); //takes integer from the id
...
function cancelTodo() {
  ...
  cancelButton.addEventListener("click", (e) => {
    e.preventDefault();
    document
      .querySelector("." + cancelButton.parentElement.parentElement.className)
      .remove(); // Selecting and removing the main wrapper
  });
}
